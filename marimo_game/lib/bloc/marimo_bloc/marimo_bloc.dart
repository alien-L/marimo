...
class MarimoBloc extends Bloc<MarimoEvent, MarimoBlocState> {
  MarimoBloc() : super(MarimoStateEmpty()) {
    on<MarimoGetInitStateEvent>(
      (event, emit) => emit(
        MarimoLevelState(
            marimoLevel: event.marimoLevel,
            marimoLifeCycle: MarimoLifeCycle.normal,
            stateScore: event.stateScore),
      ),
    );
  ...
  Future<void> getScore(
    MarimoStateScoreCalculatedEvent event,
    Emitter<MarimoBlocState> emit,
  ) async {
    String? marimoStateScoreLocalValue =
        await LocalRepository().getValue(key: "marimoStateScore");
    ...
    await LocalRepository()
        .setKeyValue(key: "marimoStateScore", value: "$stateScore");

    print("stateScore ===> $stateScore");
    return emit(MarimoLevelState(
